...
Note that, this is the development branch, the target images might not be available here, instead you can look at the [released branch](https://github.com/jboss-container-images/rhdm-7-openshift-image/tree/7.7.x/quickstarts/hello-rules)
...
[rhdm77-prod-immutable-kieserver](https://github.com/jboss-container-images/rhdm-7-openshift-image/blob/master/templates/rhdm77-prod-immutable-kieserver.yaml)
...
```bash
$ oc get template rhdm77-prod-immutable-kieserver -n openshift
Error from server (NotFound): templates "rhdm77-prod-immutable-kieserver" not found
```
If you don't have it yet, just install it:

```bash
oc create -f https://raw.githubusercontent.com/jboss-container-images/rhdm-7-openshift-image/master/templates/rhdm77-prod-immutable-kieserver.yaml -n openshift
template "rhdm77-prod-immutable-kieserver" created
```
...
Before proceed, make sure you have the RHDM imagestreams available under the 'openshift' namespace.
 ...
 ```bash
$ oc get imagestream rhdm-kieserver-rhel8 -n openshift | grep 7.7
...
```bash
$ oc create -f https://raw.githubusercontent.com/jboss-container-images/rhdm-7-openshift-image/master/rhdm77-image-streams.yaml -n openshift
```
...
```bash
$ oc new-app rhdm77-prod-immutable-kieserver \
...
-p IMAGE_STREAM_NAMESPACE=openshift
  --> Deploying template "openshift/rhdm77-prod-immutable-kieserver" to project rhdm

     Red Hat Decision Manager 7.7 immutable production environment
     ---------
     Application template for an immultable KIE server in a production environment, for Red Hat Decision Manager 7.7
...
First of all, install this quickstart on OpenShift using the [rhdm77-prod-immutable-kieserver-amq.yaml](../../templates/rhdm77-prod-immutable-kieserver-amq.yaml)
