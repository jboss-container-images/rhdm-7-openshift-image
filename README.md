...
On the root directory you can also find the two files:
 ...
 - [rhdm-image-streams.yaml](rhdm713-image-streams.yaml): imagestreams definitions, file used to install the product image streams on OpenShift, this files contains the image stream name and the registry the image will be pulled of.
...
To generate the adocs for, example, 7.13.x branch, first, switch the git branch from RHDM repository then execute the
following command:
...
It will use the `7.13.x` branch from *jboss-kie-modules* using the checked out branch on RHDM repository based from 7.13.x.
